...
#include "CustomPrintf.c"							/* include directly for now	*/

Obsoleted - use XPLRunTimeOSX.cp

Will need stubs for XPL compiler tools....

/*------------------------------------------------------------------------------*/
/* Global Variables																*/
/*------------------------------------------------------------------------------*/

static	_able_context*	global_able_context;		/* provide one context free	*/
		_able_context*  g_able_context;				/* working copy				*/

static handle	able_memory      = NULL;			/* handle for memory		*/
static handle	able_xmem		 = NULL;			/* handle for ext mem		*/
static long		able_smem_size   = 0;
	
long			g_disallow_run_host_exit;			/* disallow exit			*/
long			g_disallow_atexit;					/* disallow atexit setup	*/
...
static	short			assigned_hfs_rnum [256];	/* holds f ref num			*/
static	short			assigned_ref_count[256];	/* ref count therefore		*/
static	scsi_device*	assigned_device   [256];	/* scsi_device therefore	*/
static	FSSpec*			assigned_spec     [256];	/* FSSpec therefore			*/

/*------------------------------------------------------------------------------*/
/* Byte Swap Glue																*/
/*------------------------------------------------------------------------------*/

short	XPLRunTime_FSRead (short   refNum, long *  count, void *  buffPtr)
{
	short status;
	
	status = FSRead(refNum, count, buffPtr);
...
static	handle	gSwizzleBufferH;
#define	SWIZZLE_CHUNK_SIZE_BYTES	(128*1024)

short	XPLRunTime_FSWrite (short   refNum, long *count, const void *buffPtr)
{
	#if __BIG_ENDIAN__
		...
		*count = globDone;				// Prep return value in case of failure
		...
		if (!gSwizzleBufferH)
			gSwizzleBufferH = get_big_memory(SWIZZLE_CHUNK_SIZE_BYTES);
			
		if (!gSwizzleBufferH || !*gSwizzleBufferH)
			return (memFullErr);
			
		while (globCount)
		{
			long chunkCount = SWIZZLE_CHUNK_SIZE_BYTES;
			ufixed* dataPtr = (ufixed *) globBuf;
			ufixed* writPtr = (ufixed *) *gSwizzleBufferH;
			long i;
			long reqCount;
			...
			status = FSWrite(refNum, &reqCount, (void *) *gSwizzleBufferH);
			
			globDone += reqCount;
			
			*count = globDone;
			
			if (status || reqCount != chunkCount)
				return (status);
...
short	XPLRunTime_FSReadFork (short forkRefNum, unsigned short positionMode, long long positionOffset, unsigned long requestCount, void *buffer, unsigned long *actualCount)
{
	short status;
	
	status = FSReadFork(forkRefNum, positionMode, positionOffset, requestCount, buffer, actualCount);
	
	#if __LITTLE_ENDIAN__
	{
		ufixed * dataPtr = (ufixed *) buffer;
		long   i = (*actualCount) >> 1;
...
short	XPLRunTime_FSWriteFork (short forkRefNum, unsigned short positionMode, long long positionOffset, unsigned long requestCount, const void *buffer, unsigned long *actualCount)
{
	#if __BIG_ENDIAN__
		return (FSWriteFork(forkRefNum, positionMode, positionOffset, requestCount, buffer, actualCount));
	
	#else
		short		   status    = noErr;
		unsigned long  globCount = requestCount;
		char*		   globBuf   = (char *) buffer;
		unsigned long  globDone  = 0;
		
		*actualCount = globDone;				// Prep return value in case of failure
		
		// Only handle fsAtMark since we don't muck with position
		if (positionMode != fsAtMark || positionOffset != 0)
		...
		while (globCount)
		{
			unsigned long chunkCount = SWIZZLE_CHUNK_SIZE_BYTES;
			ufixed* dataPtr = (ufixed *) globBuf;
			ufixed* writPtr = (ufixed *) *gSwizzleBufferH;
			long i;
			unsigned long actCount = 0;
			...
			status = FSWriteFork(forkRefNum, positionMode, positionOffset, chunkCount, (void *) *gSwizzleBufferH, &actCount);
...
handle	get_big_memory(long size)
{
	OSErr 	result;
	Handle	it;
	
	it = TempNewHandle(size, &result);
...
void	free_big_memory(handle it)
{
	...
	HUnlock      ((Handle) it);
	DisposeHandle((Handle) it);
}

unsigned long	host_milliseconds()
{
	return (TickCount() * 17);
}
...
{													/* routine with special		*/
	extern int parse_printf(void (*out_routine)(char it), const char * format_str, va_list arg);
...
static	fixed	set_up_able_core()
{
	...
	if (!able_memory)
		able_memory = get_big_memory(_ABLE_MEM_WORD_SIZE << 1);
	
	if (!able_memory || !*able_memory || !g_able_context)
	{
		printf("XPL Run Time: no ABLE memory available...\n");
		return(-1);
	}

	ABLE_CONTEXT._able_memory_           = (fixed *) *able_memory;
	...
	set_able_core(c_contab + c_cmopt,  shl(3,8) | o_d4567);
...
{
	if (((ulong) where) < _ABLE_MEM_WORD_SIZE)
	{
		printf("XPL Run Time: bad location passed to host_core (0x%08x)\n", where);
		exit(-1);
	}
...
void	set_host_core(fixed *where, fixed value)
{
	if (((ulong) where) < _ABLE_MEM_WORD_SIZE)
	{
		printf("XPL Run Time: bad location passed to set_host_core (0x%08x)\n", where);
		exit(-1);
	}
...
static	fixed	set_up_able_xmem(unsigned long ext_mem_size_sectors)
{
	if (able_xmem)				/* if exists, see if is too small...			*/
	{
		if (able_smem_size < ext_mem_size_sectors)
		{
			free_big_memory(able_xmem);
			able_xmem      = NULL;
			able_smem_size = 0;
	...
	if (!able_xmem)
	{
		able_xmem      = get_big_memory(ext_mem_size_sectors << 9);
		able_smem_size = ext_mem_size_sectors;
	}
		
	if (!able_xmem || !*able_xmem || !g_able_context)
	{
		printf("XPL Run Time: no ABLE external memory available...\n");
		return(-1);
	}

	ABLE_CONTEXT.d60 = (fixed *) *able_xmem;
...
{
	if (((ulong) source) < ((ulong) dest))					/* moving up		*/
	{
...
static	void	XPL_run_time_cleanup(void)
{
	...
	if (gSwizzleBufferH)
	{
		free_big_memory(gSwizzleBufferH);
		gSwizzleBufferH = NULL;
	}
}

fixed	initialize_run_time_environment(unsigned long ext_mem_size_sectors)
{
	...
	if (!global_able_context)
		global_able_context = (_able_context*) malloc(sizeof(_able_context));
	...
	g_able_context = global_able_context;
	...
	memset(g_indexed_device,   0, sizeof(g_indexed_device  ));
	memset(g_indexed_setting,  0, sizeof(g_indexed_setting ));
...
void* open_able_file(char *file_name)
{
	...
	if ((in_file = fopen(ABLE_CONTEXT.opened_file_name, "rb")) == 0)
		{printf("XPL Run Time: can't open file '%s' ('%s')\n", file_name, ABLE_CONTEXT.opened_file_name); exit(-1);}
	
	if (getfinfo(ABLE_CONTEXT.opened_file_name, 0, &opened_finfo))
		{printf("XPL Run Time: can't get finder info for '%s' ('%s')\n", file_name, ABLE_CONTEXT.opened_file_name); exit(-1);}
...
void* open_able_file_for_output(char *file_name, long type, long creator)
{
	OSErr	FSstatus;
	char	VolName[512] = {""};
	short	VRefNum      = 0;
	...
	FSstatus = getvinfo(0, VolName, &VRefNum, &freeBytes);
	
	if (FSstatus)
		{printf("XPL Run Time: Failed getvinfo (%d)\n", FSstatus); exit(-1);}
	
	if (file_name[0] == ':')					/* file name begins with ":" : use master dir	*/
	{
		int i = strlen(ABLE_CONTEXT.able_master_dir_name);	/* get length of master directory				*/
		
		strncpy (out_file_name, ABLE_CONTEXT.able_master_dir_name, sizeof(out_file_name));
	
		if (i && out_file_name[i-1] == ':')		/* remove trailing : from master dir name		*/
			out_file_name[i-1] = 0;				/* since our file name starts with :			*/
			
		strncat (out_file_name, file_name, sizeof(out_file_name) - strlen(out_file_name));
	}
	
	else if (ABLE_CONTEXT.host_output_dir_name[0])	/* else if overriding output dir is specified	*/
	{
		strncpy (out_file_name, ABLE_CONTEXT.host_output_dir_name, sizeof(out_file_name));
		strncat (out_file_name, file_name, sizeof(out_file_name) - strlen(out_file_name));
	}
	
	else										/* else if non specified						*/
	{											/* then use mac current dir						*/
		c2pstr(VolName);						/* get vol name									*/
		
		FSstatus = HGetVol((uint8 *) VolName, &VRefNum, &dirID);
		...
		strncpy (out_file_name, ":",        sizeof(out_file_name));	/* else need colon here...	*/
		strncat (out_file_name, file_name,  sizeof(out_file_name) - strlen(out_file_name));
	}
	
	// Revert "/" in file names back to ':'
	i =  strlen(out_file_name);
	...
	out_file = fopen(out_file_name, "rb+");		/* if can open...	*/
	if (out_file)
	{
		fclose(out_file);
	...
	c2pstr(out_file_name  );					/* get in pascal format			*/

	FSstatus = HCreate(VRefNum, dirID, (uint8 *) out_file_name,   creator, type);
	
	if (FSstatus)
	{
		p2cstr((uint8 *) out_file_name  );		/* get back in c format			*/
		printf("XPL Run Time: Failed HCreate of file '%s' ('%s', %d)\n", file_name, out_file_name, FSstatus);
		exit(-1);
	}

	p2cstr((uint8 *) out_file_name  );			/* get back in c format			*/
	
	if ((out_file = fopen(out_file_name, "rb+")) == 0)
		{printf("XPL Run Time: can't open output file %s (%s)\n", file_name, out_file_name); exit(-1);}
...
void	configure_able_hard_drives (long w0_scsi_id, long w0_cyls, long w0_secs,
									long w1_scsi_id, long w1_cyls, long w1_secs)
{
...
short	find_hfs_device (ufixed device)					// find a device which is a mac HFS image file
{
...
FSSpec*	find_hfs_scsi_spec (ufixed device)				// find a fsspec which is a mac HFS image file
{
	if ((device >= ABLE_HFS_READDATA_CODE)
	&&  (device < 256)
	&&  (assigned_spec[device]))
		return (assigned_spec[device]);
	else
		return (NULL);
}
...
static	void	issue_writedata (fixed ms_sector, fixed ls_sector, fixed *_bufptr, uint32 length)
{
	...
	if (!ptr)											/* if oops				*/
	{
		...
		if ((ref_num = find_hfs_device(device))	!= 0)	// HFS Device
		{
            ...
            ByteCount	num_bytes_requested	= length << 1, num_bytes_copied = 0;

            status	= FSSetForkPosition(ref_num, fsFromStart, ((UInt64)sec) << (8+1));

            if (status == noErr)	status	= XPLRunTime_FSWriteFork(ref_num, fsAtMark, 0, num_bytes_requested, (void *)_bufptr, &num_bytes_copied);
...
static	void	issue_readdata (fixed ms_sector, fixed ls_sector, fixed *_bufptr, uint32 length)
{
	...
	if (!ptr)											/* if oops				*/
	{
		...
		{		
            ...
            ByteCount	num_bytes_requested	= length << 1,
                            num_bytes_copied		= 0;

            status	= FSSetForkPosition(ref_num, fsFromStart, ((UInt64)sec) << (8+1));

            if (status == noErr)
             status	= XPLRunTime_FSReadFork(ref_num, fsAtMark, 0, num_bytes_requested, (void *)_bufptr, &num_bytes_copied);
...
void	XPLRunTime_SetupSCSIMap(interchange_settings *interchangeSettings,  unsigned long dev24_read_code)
{
	...
	&&   (dev24_read_code                    != 0                          )))			// and we have the hardware available
	{
...
void	XPLRunTime_ConfigureSCSIMap(fixed floppy_available)
{
	int  i = c_contab + c_strdev;						// put W0 at first entry in storage area
	scsi_device 	*the_device;
...
//	Note: the following two function pointers are no longer used due to switch to 64-bit carbon file I/O routines
short   (*XPLRunTime_FileOpener) (struct FSSpec *inFileSpec, char permission, short *refNum) = NULL;
short   (*XPLRunTime_FileCloser) (short refNum)                                              = NULL;
...
scsi_device*	XPLRunTime_LookUpDevicePointer(long theBoard, long theScsiID)
{
...
scsi_settings*	XPLRunTime_LookUpDeviceSettings(long theBoard, long theScsiID)
{
	MAC_SCSI_Target *the_target = NULL;
...
MAC_SCSI_Target*	XPLRunTime_ExamineDevice(long theBoard, long theScsiID)
{
	MAC_SCSI_Target *the_target = NULL;
...
void XPLRunTime_CloseupSCSIMap()
{
	...
	for (i=0; i<NUM_BOARDS_IN_BOARD_LIST; i++)
	{
		for (j=0; j<NUM_TARGETS_IN_TARGET_LIST; j++)
		{
			...
			{
				if (XPL_scsi_code_map.board_list[i].target_list[j].ref_num)
				{
...
ufixed	XPLRunTime_AssignHFSReaddataCode (short  inFileRefNum, struct FSSpec *inFileSpec)
{
	int i;
	...
	FSSpec     * its_spec   = (FSSpec*     ) malloc(sizeof(FSSpec     ));
	...
	memset(its_device, 0, sizeof(scsi_device));						// init scsi struct
...
ufixed XPLRunTime_LookForFSSpecInUse(const struct FSSpec *inFileSpec)
{
	int i,j;
	
	if (inFileSpec->name[0] == 0)
		return (0);
	...
	for (i=ABLE_HFS_READDATA_CODE; i<256; i++)
	{
		if (assigned_spec[i])
		{
			FSSpec &itsSpec = *assigned_spec[i];
			
			if (itsSpec.vRefNum != inFileSpec->vRefNum)
				continue;
				
			if (itsSpec.parID   != inFileSpec->parID)
				continue;
				
			if (itsSpec.name[0] != inFileSpec->name[0])
				continue;
				
			for (j=1; j<=itsSpec.name[0]; j++)
			{
				if (itsSpec.name[j] != inFileSpec->name[j])
					break;
			}
